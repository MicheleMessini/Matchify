...
app.get('/playlist/:id', async (req, res) => {
  ...
  try {
    ...
    // Recupera tutte le tracce della playlist (paginazione)
    let tracks = [];
    ...
    // Raggruppa le tracce per album (escludendo singoli e podcast)
    const albumsMap = new Map();

    for (const item of tracks) {
      const track = item.track;
      if (!track || !track.album) continue;
      // Escludi podcast e singoli (album_type !== 'album')
    ...
    // Prepara dati album con conteggio tracce presenti nella playlist
    ...
    // Ordina album per numero di tracce presenti nella playlist (desc)
